...
so server.py and the frontend parser never change when AGENT_MODE flips:
...
Backend implementation: Python `httpx` + `httpx-sse` for the Snowflake client,
`fastapi` + `sse-starlette.EventSourceResponse` for the server stream. Both the
mock and Snowflake clients are async generators yielding dict objects of the
canonical shape `{"event": <EVENTS.*>, "data": {...}}`.
